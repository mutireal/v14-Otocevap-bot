...
client.on('interactionCreate', async (interaction) => {
  ...
  if(interaction.customId === 'form') {
    ...
    muti.set(`otomesaj_${modalMessage}${interaction.guildId}`,  { modalMessage: modalMessage, modalCreate: modalCreate })


    await interaction.reply({ embeds: [embed], fetchReply: true, ephemeral: true}).then(() => {
      muti.push(`cmds-${interaction.guildId}`, `${modalMessage}-${modalCreate}`)
    }).catch(() => {})
  }
  ...
  if(interaction.customId === 'form1') {
    ...
    await interaction.reply({ embeds: [embed], fetchReply: true, ephemeral: true}).then(() => {
      muti.unpush(`cmds-${interaction.guildId}`, `${modalMessage}-${ muti.fetch(`otomesaj_${modalMessage}${interaction.guildId}`).modalCreate}`)
      muti.delete(`otomesaj_${modalMessage}${interaction.guildId}`)
    }).catch(() => {})
  }
...
client.on("interactionCreate", async interaction => {
  if(interaction.customId === "otomesaj") {
    ...
    .setDescription(`${muti.fetch(`cmds-${interaction.guildId}`).map(c => `${c.split("-")[0]} **-** ${c.split("-")[1]}`).join("\n")}\n`)
  ...
  if(interaction.isChatInputCommand()) {
    ...
    readdirSync('./commands').forEach(f => {
      ...
      if(interaction.commandName.toLowerCase() === cmd.name.toLowerCase()) {
        return cmd.run(client, interaction, muti);
      }
});
}
};
